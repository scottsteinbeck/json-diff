component singleton {
  ...
  function diffByKey(
    ...
    var groupedDiff = diffData.reduce( ( acc, x ) => {
      if ( x.type == "add" ) {
        ...
        acc[ x.type ][ x.path[ 1 ] ].append( { "key" : pathRest[ 1 ], "path" : pathRest, "new" : x.new, "old" : x.old } );
      }
      return acc
    }, { "add" : [], "remove" : [], "change" : {} } );
    groupedDiff[ "update" ] = groupedDiff.change.reduce( ( acc, key, value ) => {
      ...
      acc.push( { "key" : key, "orig" : data1[ key ], "data" : data2[ key ], "changes" : value } )
      return acc;
    }, [] );
  ...
  function diff( any first = "", any second = "", array ignoreKeys = [] ) {
    ...
    if (
      ...
      diffs.append( { "path" : [], "type" : "CHANGE", "old" : first, "new" : second } );
      ...
      for ( var i = 1; i <= first.len(); i++ ) {
        ...
        if ( second.len() < i ) {
          diffs.append( { "path" : [ path ], "type" : "REMOVE", "old" : first[ i ], "new" : "" } );
          ...
          if (
            ...
            if ( precisionEvaluate( first[ i ] - second[ i ] ) != 0 ) {
              diffs.append( { "path" : [ path ], "type" : "CHANGE", "old" : first[ i ], "new" : second[ i ] } );
            }
      ...
      for ( var t = first.len() + 1; t <= second.len(); t++ ) {
        var path = t;
        diffs.append( { "type" : "ADD", "path" : [ path ], "old" : "", "new" : second[ path ] } );
      }
      ...
      for ( var key in first ) {
        ...
        if ( !second.keyExists( key ) ) {
          diffs.append( { "path" : [ path ], "type" : "REMOVE", "old" : first[ key ], "new" : "" } );
          ...
          if (
            ...
            if ( precisionEvaluate( first[ key ] - second[ key ] ) != 0 ) {
              diffs.append( { "key" : path, "path" : [ path ], "type" : "CHANGE", "old" : first[ key ], "new" : second[ key ] } );
            }
      ...
      for ( var key2 in second ) {
        ...
        if ( structKeyExists( second, key2 ) && !structKeyExists( keysSeen, key2 ) ) {
          diffs.append( { "type" : "ADD", "path" : [ key2 ], "old" : "", "new" : second[ key2 ] } );
        }
      }
    }
